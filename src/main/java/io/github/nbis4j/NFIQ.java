        ...
        switch (format) {
            case BMP:
                ...
                try {
                    ...
                    if (status != 0) {
                        throw new NFIQComputeException("Error in native layer", status);
                    }
                    return nfiq.getValue();
                } catch (IOException e) {
                    throw new NFIQComputeException("Error reading BMP", e, 1000);
                }
            case WSQ:
                IntByReference nfiq = new IntByReference();
                ...
                return nfiq.getValue();
            default:
                throw new NFIQComputeException("Unsupported image format", 1000);
        }
    }
